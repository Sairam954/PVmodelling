...
def getAggregateBlkLoss(aggre_blk,wavelength_idx,lamdar_per_channel):
    ...
    aggre_blk_loss = aggr_mr_drop_loss + aggr_wvg_loss + aggr_through_loss+getCouplingLoss()
...
def getResolution(mr_0):
    ...
    mr_pv_ER = mr_0['ER']
    resolution = np.ceil(np.log2(resolution * (mr_pv_ER / DESIGN_ER))).astype(int)


    # resolution = int(resolution * (mr_pv_ER / DESIGN_ER))
    return resolution
def getParameterDieFiles(path):
